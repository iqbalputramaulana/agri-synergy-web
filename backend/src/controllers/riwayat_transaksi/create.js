...
const validateFields = {
  ...
  validateData: (req) => {
    const { id_user, id_pengiriman, id_memesan, metode_pembayaran } = req.body;
...
module.exports = async (req, res) => {
  try {
    ...
    const [row] = await req.db
      .promise()
      .query("INSERT INTO riwayat_transaksi SET ?", validation.data);
      ...
      return res
        ...
        .json(
          RESPONSE.createSuccess(
            row,
            "Data riwayat transaksi berhasil ditambahkan"
          )
        );
    } else {
      return res
        .status(400)
        .json(
          RESPONSE.createError(400, "Data riwayat transaksi gagal ditambahkan")
        );
    }
  } catch (err) {
    console.error(err);
