...
const FILE_TYPES = {
  ...
  UPLOAD_PATH: path.join(__dirname, "../../files/petaLahan"),
};
...
const uploadMiddleware = (req, res, next) => {
  const upload = imageUploader.single("foto_lokasi");
  ...
  upload(req, res, (err) => {
    ...
    if (!req.file) {
      req.body.foto_lokasi = null; 
      return next();
    }

    req.body.foto_lokasi = req.file.filename;
    next();
  });
};
...
const validateStaticFile = (req, res, next) => {
  ...
  if (!allowedExt.includes(ext)) {
    return res.status(403).json(
      RESPONSE.createError(403, "Format file tidak diizinkan")
    );
  }
  next();
};
